...
调用 API 前请确认是否需要对应权限，在对应模块的module.json5中确认权限配置
如需使用依赖库，请确认依赖库的存在和匹配版本，并在对应模块的oh-package.json5中添加依赖配置
使用@Component和@ComponentV2时需要区分兼容性，尽量与已有工程代码保持一致
UI 界面展示引用的常量需要定义 resources 资源值，并使用$r引用, 一般不直接使用字面值
